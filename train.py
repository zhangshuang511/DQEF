...
from model.model import Classifer 
import numpy as np
import torch
...
def train(model,train_set, optimizer, scheduler=None, batch_size=32):
    ...
    for i, batch in enumerate(iterator):
        ...
        _, xs, y, _, masks,left_zs, right_zs = batch
        _y = y

        optimizer.zero_grad()

        logits, y, a, class_loss, logits_aug, y_aug, contrastive_loss= model(xs, y, masks,left_zs, right_zs)
                
        logits = logits.view(-1, logits.shape[-1])
        y = y.view(-1)
        loss_ori = classifier_criterion(logits, y)   
        
        logits_aug = logits_aug.view(-1, logits_aug.shape[-1])
        loss_aug = classifier_criterion(logits_aug, y) + classifier_criterion(logits, y)  

        loss = loss_ori + 1 * loss_aug + 0* class_loss + 0.5*contrastive_loss
        loss.backward()
        optimizer.step()
        ...
        if i % 10 == 0:
            print(f"step: {i}, loss: {loss.item()}, loss_ori: {loss_ori.item()}, loss_aug: {loss_aug.item()}, class_loss: {class_loss.item()}, contrastive_loss: {contrastive_loss.item()}")
            del loss
...
def initialize_and_train(trainset, validset, testset, attr_num, args, run_tag):
    ...
    if device == 'cpu':
        ...
        # optimizer_class = AdamW(class_model.parameters(), lr=args.lr)
    else:
        model = model.cuda()
        optimizer = AdamW(model.parameters(), lr=args.lr)
        # optimizer_class = AdamW(class_model.parameters(), lr=args.lr)
