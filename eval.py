...
def eval_classifier(model, iterator):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            _, x, y, _, masks,left_zs, right_zs = batch
            logits, y1, y_hat,igo, logits_aug, y_aug, contrastive_loss = model(x, y, masks,left_zs, right_zs)
...
def eval_on_task(epoch, model, valid_iter, test_iter,
                 writer, run_tag):
    print('Validation:')
    start = time.time()
